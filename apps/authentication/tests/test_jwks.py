...
def test_jwks_key_verifies_a_real_login_token(api_client: APIClient) -> None:
    ...
    login_response = api_client.post("/api/v1/auth/login/", {"username": "jwksuser", "password": PASSWORD})
...
def test_access_token_header_kid_matches_jwks(api_client: APIClient) -> None:
    ...
    login_response = api_client.post("/api/v1/auth/login/", {"username": "kiduser", "password": PASSWORD})
    access = login_response.data["access"]
